	...
	for(int i = 0; i < sift.keyPointVec.size(); ++i)
	{
		...
		*/
		...
		for(int s = 0; s < orient.size(); ++s)
		{
			...
			int bin = cvRound(orient.at(s) / 10);
...
void SIFT_Descriptor::MakeDescriptor(SIFT &sift)
{
	...
	for(int i = 0; i < sift.keyPointVec.size(); ++i)
	{
		...
		*/
		...
		scale : 스케일(실습에서는 1)
		*/
		cv::Mat m = getRotationMatrix2D(cv::Point(img.rows/2, img.cols/2), orientation, 1);
		...
		{			
			for(int w = beginPoint.x; w < endPoint.x; w+= siz)
			{
				...
				{			
					...
					{						
							...
							& rotationIdxY > 0 && rotationIdxY < img.rows - 1 )
						{
							...
							float dx = img.at<uchar>(rotationIdxY, rotationIdxX + 1) - img.at<uchar>(rotationIdxY, rotationIdxX - 1);
							float dy = img.at<uchar>(rotationIdxY + 1, rotationIdxX) - img.at<uchar>(rotationIdxY - 1, rotationIdxX);
				...
				for(int s = 0; s < orient.size(); ++s)
				{
					...
					int bin = cvRound(orient.at(s) / 45);
					if( bin >= SIFT_ORI_HIST_BINS )
						bin -= SIFT_ORI_HIST_BINS;
					if( bin < 0 )
						bin += SIFT_ORI_HIST_BINS;
				...
				float sum_of_hist = 0;
				// 정규화
				for (int s = 0; s < SIFT_ORI_HIST_BINS;s++) {
					sum_of_hist += hist[s];
				}
				for (int s = 0; s < SIFT_ORI_HIST_BINS;s++) {
					if(sum_of_hist != 0)
						hist[s] = hist[s] / sum_of_hist;
				}
